...
def delete(
    ...
    try:
        ...
        if pretty_output:
            ...
            if not force:
                confirm = typer.confirm(f"[{Colors.ERROR}]Are you sure you want to delete this issue?[/{Colors.ERROR}]")
                if not confirm:
                    console.print(f"[{Colors.WARNING}]ℹ[/{Colors.WARNING}] Deletion cancelled.")
                    return
        else:
...
def _display_deletion_preview(issue: dict):
    ...
    if issue.get('severity'):
        ...
        content.append(issue['severity'], style=Styles.get_severity_color(issue['severity']))
        content.append("\n")
