...
class TestCreateLlmChain:
    ...
    def test_creates_openai_chain_with_config(self, mock_load_config, mock_chat_openai):
        ...
        mock_load_config.return_value = {"model": "gpt-4", "openai_api_key": "test-key"}
...
class TestRetryLogic:
    ...
    def test_should_retry_with_success(self):
        ...
        state = ProcessingState(
            input_description="Test", processed_result={"title": "Success"}
        )
    ...
    def test_handle_retry_logic_increments_count(self):
        ...
        state = ProcessingState(input_description="Test", retry_count=1, max_retries=3)
...
class TestProcessDescription:
    ...
    @patch("core.model.load_config")
    def test_processes_description_successfully(
        self, mock_load_config, mock_create_graph
    ):
        """Test successful processing of bug description"""
        mock_load_config.return_value = {"openai_api_key": "test-key", "retry_limit": 3}
...
class TestSetupLanggraph:
    ...
    def test_sets_up_langgraph_successfully(self, mock_load_config, mock_create_llm):
        ...
        mock_load_config.return_value = {"openai_api_key": "test-key"}
...
class TestModelErrorPaths:
    ...
    def test_validate_empty_description_title_generation(self):
        ...
        result2 = validate_and_clean_result(
            {"title": "", "severity": "medium", "type": "bug", "tags": []}, ""
        )
    ...
    @patch("core.model.create_processing_graph")
    @patch("core.model.load_config")
    def test_process_description_missing_result_error(
        self, mock_load_config, mock_create_graph
    ):
    ...
    @patch("core.model.create_processing_graph")
    @patch("core.model.load_config")
    def test_process_description_general_exception_wrapping(
        self, mock_load_config, mock_create_graph
    ):
        ...
        with pytest.raises(
            ModelError, match="LLM processing failed: Unexpected runtime error"
        ):
            process_description("Test description")

    @patch("core.model.create_llm_chain")
    @patch("core.model.load_config")
    def test_setup_langgraph_connection_test_error(
        self, mock_load_config, mock_create_llm
    ):
        """Test lines 329-331: Error during connection testing in setup_langgraph"""
        mock_load_config.return_value = {"openai_api_key": "sk-test-key"}
...
class TestModelValidationEdgeCases:
    ...
    def test_validate_non_list_tags(self):
        ...
        # Test with None tags
        result2 = validate_and_clean_result(
            {"title": "Test", "severity": "high", "type": "feature", "tags": None},
            "Test description",
        )
    ...
    def test_handle_retry_logic_line_221_exact_coverage(self):
        ...
        assert "Last error: Final error that will be in exception" in str(
            exc_info.value
        )
