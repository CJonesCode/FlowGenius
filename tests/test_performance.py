...
class TestStoragePerformance:
    ...
    def test_large_issue_list_performance(
        self, temp_dir, issue_factory, performance_monitor
    ):
    ...
    def test_memory_usage_with_large_dataset(self, temp_dir, issue_factory):
        ...
        for i in range(num_issues):
            issue = issue_factory.create_issue(
                ...
                description=f"This is a longer description for memory test issue {i+1}. "
                * 10,
            ...
            if (i + 1) % 100 == 0:
                ...
                assert (
                    memory_growth < 100
                ), f"Memory growth too large: {memory_growth:.2f}MB after {i+1} issues"
        ...
        assert (
            total_growth < 200
        ), f"Total memory growth too large: {total_growth:.2f}MB"
...
class TestModelPerformance:
    ...
    def test_model_processing_batch_performance(
        self, mock_openai_client, mock_config_operations
    ):
        ...
        ), f"Average processing time too slow: {avg_time_per_request:.3f}s"
    ...
    def test_model_processing_retry_performance(self, mock_config_operations):
        ...
        for max_retries in retry_counts:
            with patch("core.model.ChatOpenAI") as mock_openai:
                ...
                assert (
                    retry_time < 1.0
                ), f"Retry scenario took too long: {retry_time:.3f}s for {max_retries} retries"
...
class TestSchemaValidationPerformance:
    ...
    def test_bulk_validation_performance(self, performance_monitor):
        ...
        ), f"Validation too slow: {avg_time_per_validation:.4f}s per item"
    ...
    def test_validation_edge_cases_performance(self):
        ...
        assert (
            edge_case_time < 0.1
        ), f"Edge case validation too slow: {edge_case_time:.3f}s"
...
class TestIntegrationPerformance:
    ...
    def test_full_workflow_performance(
        self, temp_dir, mock_openai_client, mock_config_operations
    ):
        ...
        assert (
            avg_time_per_workflow < 0.1
        ), f"Workflow too slow: {avg_time_per_workflow:.3f}s per iteration"
