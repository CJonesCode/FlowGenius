...
class TestSaveIssue:
    ...
    def test_overwrites_existing_issue(self, temp_dir):
        ...
        initial_data = {"id": issue_id, "title": "Initial Title", "severity": "low"}
        save_issue(initial_data)

        # Save updated version
        updated_data = {"id": issue_id, "title": "Updated Title", "severity": "high"}
    ...
    def test_handles_special_characters_in_id(self, temp_dir):
        ...
        issue_data = {"id": "issue-with_special123", "title": "Special ID Test"}
...
class TestAtomicWriteErrorPaths:
    ...
    @pytest.mark.skipif(
        sys.platform.startswith("win"), reason="Permission testing complex on Windows"
    )
    ...
    def test_atomic_write_temp_file_cleanup_on_error(self, temp_dir):
        ...
        with patch(
            "core.storage.os.fdopen", side_effect=OSError("Simulated write error")
        ):
...
class TestSaveIssueErrorPaths:
    ...
    def test_save_issue_general_exception_handling(self, temp_dir):
        ...
        with patch(
            "core.storage.atomic_write_json", side_effect=Exception("Unexpected error")
        ):
    ...
    def test_file_lock_exception_handling(self, temp_dir):
        ...
        if sys.platform.startswith("win"):
            ...
            with patch("msvcrt.locking", side_effect=mock_locking):
                with pytest.raises(
                    ConcurrentAccessError, match="Could not acquire lock"
                ):
                    with storage.file_lock(test_file, timeout=0.1):
                        pass
        else:
