...
class TestNewCommand:
    ...
    def test_new_issue_json_output(self):
        ...
        with patch('core.model.process_description', return_value=self.sample_processed) as mock_process, \
             patch('core.schema.validate_or_default', return_value=self.sample_validated) as mock_validate, \
             patch('core.storage.save_issue', return_value='new123') as mock_save:
            
            result = self.runner.invoke(app, ["new", "User cannot login to the application"])
    ...
    def test_new_issue_pretty_output(self):
        ...
        with patch('core.model.process_description', return_value=self.sample_processed) as mock_process, \
            ...
            result = self.runner.invoke(app, ["new", "User cannot login to the application", "--pretty"])
            ...
            assert 'Issue created: new123' in result.stdout
            assert 'Title: Login button not working' in result.stdout
            ...
            assert 'Saved to: .bugit/issues/new123.json' in result.stdout
    
    def test_new_issue_short_flag(self):
        """Test creating new issue with short flag (-p)"""
        with patch('core.model.process_description', return_value=self.sample_processed), \
             patch('core.schema.validate_or_default', return_value=self.sample_validated), \
             patch('core.storage.save_issue', return_value='new123'):
    ...
    def test_new_issue_with_no_tags_pretty(self):
        ...
        with patch('core.model.process_description', return_value=processed_no_tags), \
             patch('core.schema.validate_or_default', return_value=validated_no_tags), \
             patch('core.storage.save_issue', return_value='notags123'):
    ...
    def test_new_issue_with_missing_tags_field_pretty(self):
        ...
        with patch('core.model.process_description', return_value=processed_missing_tags), \
             patch('core.schema.validate_or_default', return_value=validated_missing_tags), \
             patch('core.storage.save_issue', return_value='missing123'):
            
            result = self.runner.invoke(app, ["new", "Bug with missing tags field", "--pretty"])
    ...
    def test_new_issue_different_severities_pretty(self):
        ...
        for severity in severities:
            ...
            with patch('core.model.process_description', return_value=processed), \
                 patch('core.schema.validate_or_default', return_value=validated), \
                 patch('core.storage.save_issue', return_value=f'{severity}123'):
                
                result = self.runner.invoke(app, ["new", f"Bug with {severity} severity", "--pretty"])
    ...
    def test_new_issue_long_description(self):
        ...
        with patch('core.model.process_description', return_value=self.sample_processed) as mock_process, \
             patch('core.schema.validate_or_default', return_value=self.sample_validated), \
             patch('core.storage.save_issue', return_value='long123'):
    ...
    def test_new_issue_special_characters(self):
        ...
        with patch('core.model.process_description', return_value=self.sample_processed) as mock_process, \
             patch('core.schema.validate_or_default', return_value=self.sample_validated), \
             patch('core.storage.save_issue', return_value='special123'):
            ...
            assert 'Issue created: special123' in result.stdout
    
    def test_new_issue_model_error_json_mode(self):
        """Test new command with model processing error in JSON mode"""
        with patch('core.model.process_description', side_effect=Exception("LLM API error")):
            ...
            assert 'Error creating issue: LLM API error' in output['error']
    
    def test_new_issue_model_error_pretty_mode(self):
        """Test new command with model processing error in pretty mode"""
        with patch('core.model.process_description', side_effect=Exception("Model timeout")):
            ...
            assert 'Error creating issue: Model timeout' in result.stdout
    
    def test_new_issue_validation_error_json_mode(self):
        """Test new command with validation error in JSON mode"""
        with patch('core.model.process_description', return_value=self.sample_processed), \
             patch('core.schema.validate_or_default', side_effect=Exception("Validation failed")):
    ...
    def test_new_issue_validation_error_pretty_mode(self):
        ...
        with patch('core.model.process_description', return_value=self.sample_processed), \
             patch('core.schema.validate_or_default', side_effect=Exception("Invalid schema")):
            ...
            # Verify pretty error output
            assert 'Error creating issue: Invalid schema' in result.stdout
    
    def test_new_issue_storage_error_json_mode(self):
        """Test new command with storage error in JSON mode"""
        with patch('core.model.process_description', return_value=self.sample_processed), \
             patch('core.schema.validate_or_default', return_value=self.sample_validated), \
             patch('core.storage.save_issue', side_effect=Exception("Disk full")):
    ...
    def test_new_issue_storage_error_pretty_mode(self):
        ...
        with patch('core.model.process_description', return_value=self.sample_processed), \
             patch('core.schema.validate_or_default', return_value=self.sample_validated), \
             patch('core.storage.save_issue', side_effect=Exception("Permission denied")):
            ...
            # Verify pretty error output
            assert 'Error creating issue: Permission denied' in result.stdout
    
    def test_new_issue_empty_description(self):
        """Test creating issue with empty description"""
        with patch('core.model.process_description', return_value=self.sample_processed) as mock_process, \
             patch('core.schema.validate_or_default', return_value=self.sample_validated), \
             patch('core.storage.save_issue', return_value='empty123'):
    ...
    def test_new_issue_single_word_description(self):
        ...
        with patch('core.model.process_description', return_value=self.sample_processed) as mock_process, \
             patch('core.schema.validate_or_default', return_value=self.sample_validated), \
             patch('core.storage.save_issue', return_value='single123'):
    ...
    def test_new_issue_whitespace_description(self):
        ...
        with patch('core.model.process_description', return_value=self.sample_processed) as mock_process, \
             patch('core.schema.validate_or_default', return_value=self.sample_validated), \
             patch('core.storage.save_issue', return_value='whitespace123'):
    ...
    def test_new_issue_multiline_description(self):
        ...
        with patch('core.model.process_description', return_value=self.sample_processed) as mock_process, \
             patch('core.schema.validate_or_default', return_value=self.sample_validated), \
             patch('core.storage.save_issue', return_value='multiline123'):
    ...
    def test_new_issue_different_types_pretty(self):
        ...
        for issue_type in types:
            ...
            with patch('core.model.process_description', return_value=processed), \
                ...
                result = self.runner.invoke(app, ["new", f"Issue of type {issue_type}", "--pretty"])
    ...
    def test_new_issue_many_tags_pretty(self):
        ...
        with patch('core.model.process_description', return_value=processed_many_tags), \
             patch('core.schema.validate_or_default', return_value=validated_many_tags), \
             patch('core.storage.save_issue', return_value='manytags123'):
            
            result = self.runner.invoke(app, ["new", "Bug with many tags", "--pretty"])
            
            assert result.exit_code == 0
            assert 'Tags: tag0, tag1, tag2, tag3, tag4, tag5, tag6, tag7, tag8, tag9' in result.stdout
    ...
    def test_new_command_help(self):
        ...
        assert "Freeform text description of the bug" in result.stdout
    
    def test_new_issue_json_output_structure(self):
        """Test that JSON output has correct structure"""
        with patch('core.model.process_description', return_value=self.sample_processed), \
             patch('core.schema.validate_or_default', return_value=self.sample_validated), \
             patch('core.storage.save_issue', return_value='structure123'):
