...
def mock_storage_operations():
    ...
    with patch('core.storage.save_issue') as mock_save, \
         patch('core.storage.load_issue') as mock_load, \
         patch('core.storage.list_issues') as mock_list, \
         patch('core.storage.delete_issue') as mock_delete, \
         patch('core.storage.get_issue_by_index') as mock_get_by_index:
        ...
        mock_get_by_index.return_value = {'id': 'mocked-issue-id', 'title': 'Mocked Issue'}
...
def mock_config_operations(mock_config):
    ...
    with patch('core.config.load_config') as mock_load_config, \
         patch('core.config.get_config_value') as mock_get_config, \
         patch('core.config.save_preferences') as mock_save_prefs, \
         patch('core.config.set_api_key') as mock_set_api_key, \
         patch('core.config.set_preference') as mock_set_pref, \
         patch('core.config.check_openai_api_key') as mock_check_key:
...
@pytest.fixture
def isolated_test_environment(temp_dir, clean_env, mock_config_operations, mock_storage_operations):
