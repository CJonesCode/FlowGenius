...
class TestSchemaValidationParameterized:
    ...
    @pytest.mark.parametrize("title,expected_length,should_truncate", [
    ...
    @pytest.mark.parametrize("tags,expected", [
        ...
        (["tag1", "tag2", "tag3", "tag4", "tag5", "tag6", "tag7", "tag8", "tag9", "tag10", "tag11"], 
         ["tag1", "tag2", "tag3", "tag4", "tag5", "tag6", "tag7", "tag8", "tag9", "tag10"]),  # Max 10 tags
    ])
...
class TestModelProcessingParameterized:
    ...
    @pytest.mark.parametrize("description,expected_severity", [
    ...
    def test_severity_detection(self, description, expected_severity, mock_openai_client, mock_config_operations):
    ...
    @pytest.mark.parametrize("description,expected_tags", [
    ...
    def test_tag_generation(self, description, expected_tags, mock_openai_client, mock_config_operations):
...
class TestStorageOperationsParameterized:
    ...
    @pytest.mark.parametrize("issue_id,valid", [
        ("simple-id", True),
        ("id-with-dashes", True),
    ...
    def test_issue_id_handling(self, issue_id, valid, temp_dir, issue_factory):
        ...
        test_issue = issue_factory.create_issue(
            title="Test Issue",
            issue_id=issue_id
        )
    ...
    @pytest.mark.parametrize("num_issues,severity_filter,expected_count", [
        (0, None, 0),
        (5, None, 5),
        (10, "critical", 2),  # Assume 2 critical issues out of 10
        (3, "low", 1),        # Assume 1 low issue out of 3
    ])
    def test_list_issues_filtering(self, num_issues, severity_filter, expected_count, temp_dir, issue_factory):
        ...
        if severity_filter:
            filtered_issues = [issue for issue in all_issues if issue['severity'] == severity_filter]
...
class TestErrorScenarios:
    ...
    def test_api_error_handling(self, error_type, expected_message, mock_config_operations, simulate_api_error):
...
class TestConfigurationScenarios:
    ...
    @pytest.mark.parametrize("config_key,test_value,expected", [
    ...
    def test_config_value_handling(self, config_key, test_value, expected):
        ...
        with patch('core.config.get_config_value') as mock_get, \
             patch('core.config.set_preference') as mock_set:
    ...
    @pytest.mark.parametrize("provider,api_key,should_succeed", [
