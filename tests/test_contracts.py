...
class TestCLIContractInterface:
    ...
    def test_cli_help_output_contract(self):
        ...
        for command in essential_commands:
            assert command in help_output, f"Command '{command}' missing from help output"
    ...
    def test_cli_command_help_contracts(self):
        ...
        for command in commands:
            ...
            assert "usage:" in help_output, f"Command '{command}' help missing usage info"
            
            # Contract: Essential flags should be documented
            if command in ["new", "list", "show", "edit", "delete", "config"]:
                assert "--pretty" in help_output or "-p" in help_output, f"Command '{command}' missing --pretty flag"
...
class TestJSONSchemaContracts:
    ...
    def test_issue_schema_contract(self):
        ...
        required_fields = [
            "id", "schema_version", "title", "description", 
            "severity", "type", "tags", "created_at"
        ]
        ...
        for field in required_fields:
            assert field in result, f"Required field '{field}' missing from validated data"
        ...
        assert result['severity'] in valid_severities, f"Severity must be one of {valid_severities}"
    ...
    def test_config_schema_contract(self):
        ...
        with patch('core.config.load_config') as mock_load:
            ...
            assert isinstance(config['output_format'], str), "Output format must be string"
...
class TestFileFormatContracts:
    ...
    def test_issue_file_format_contract(self, temp_dir, sample_issue):
        ...
        required_fields = ["id", "schema_version", "title", "description", "severity", "type", "tags", "created_at"]
        for field in required_fields:
            assert field in file_data, f"Saved file missing required field: {field}"
        
        # Contract: Schema version must be correct
        assert file_data["schema_version"] == "v1", "Saved file must have correct schema version"
    ...
    def test_directory_structure_contract(self, temp_dir):
        ...
        assert issues_dir.exists() and issues_dir.is_dir(), "issues directory must exist"
...
class TestBackwardCompatibilityContracts:
    ...
    def test_schema_v1_compatibility(self, temp_dir):
        ...
        for key, value in v1_issue.items():
            assert loaded_issue[key] == value, f"v1 field '{key}' not preserved during load"
    ...
    def test_cli_flag_compatibility(self):
        ...
        for flag in essential_flags:
            if flag in ["--help", "-h"]:
                ...
                assert result.returncode == 0, f"Config command should support {flag} flag"
...
class TestAPIContractValidation:
    ...
    def test_storage_interface_contract(self):
        ...
        with patch('core.storage.list_issues') as mock_list:
            ...
            if result:
                assert isinstance(result[0], dict), "list_issues must return list of dicts"
...
class TestErrorContractConsistency:
    ...
    def test_error_message_format_contract(self):
        ...
        for error in test_errors:
            ...
            assert isinstance(error, Exception), "All errors should inherit from Exception"
